...
Course_options = np.array([['Exercise Physiology','', 0],
                            ...
                            ['Sport Coaching & Coaching Ecosystems', '', 0],
...
role_rank_options = [   "Monitor, assess and improve physical activity and dietary behaviour", # in relation to current policy (removed)
                        ...
                        "Design, deliver and evaulate programs to improve specific paramaters of health, wellbeing and performance capacity",
...
for question, alternatives in remainder_questions.items():
    ...
    if question == 'At what level do you want to work with people?':
        ...
        if response == 2:
            ...
            major_weighting['sp_n'] *=     1 + value_multiplier * 0.5
        elif response == 3:
            ...
            major_weighting['sp_n'] *=     1 + value_multiplier * 1
...
majors = ['Strength and Conditioning', 'Exercise Physiology', 'Sport Coaching & Coaching Ecosystems', 'Physical Activity and Health', 'Applied Sport Science', 'Sports Nutrition']
q_result = []
...
top_4 = []
for field in results_df['Field'][0:4]:
    top_4.append(field)
print(f'\n Your top four results were: \n 1) {top_4[0]} \n 2) {top_4[1]} \n 3) {top_4[2]} \n 4) {top_4[3]}')
...
#will rank single and double majors into a list of pssible options
final_list = []
...
#provides only the double majors as a suggestion
final_list2 = []

for index in Possible_course_options.index[0:5]:
    if Possible_course_options['Major 1'][index] != '' and Possible_course_options['Major 2'][index] != '':
        output = Possible_course_options['Major 1'][index] + ' + ' + Possible_course_options['Major 2'][index] + ' - Double Major'
        final_list2.append(output)
dmaj_list_length = list(range(0,len(final_list2)))
#print(dmaj_list_length)
print(f'\n There are some double majors available that may be interesting for you based off your answers. Here are some suggestions based off the information you have provided: \n ')
for label, value in enumerate(dmaj_list_length):
    label += 1
    print(f'{label}) {final_list2[value]} \n')

print(f'There are also minors (4 units) available for all pathways opposed to the majors (6 units) that you can accompany a major in the Bachelor of Exercise and Sport Science. Use your results to create your ideal personalised course! ')